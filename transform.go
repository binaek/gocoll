...
func Map[T comparable, U comparable](collection Collection[T], f Mapper[T, U]) Collection[U] {
	result := New[U]()
...
func FlatMap[T comparable, U comparable](collection Collection[T], f FlatMapper[T, U]) Collection[U] {
	result := New[U]()
	for _, e := range collection.Elements() {
		u := f(e)
		result.Concat(u)
